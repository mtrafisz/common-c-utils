...
Array array_ex(size_t sizeof_element, uint32_t capacity, Allocator allocator) {
    assert(capacity != 0);
...
void array_sort(ArrayPtr arr, int(*compare)(const void* __A, const void* __B)) {
    qsort(arr->data, arr->size, arr->sizeof_element, compare);
}
